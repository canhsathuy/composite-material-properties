program beam_with_core
    ...
    real*8 :: E_roh = 75d6
    real*8 :: G12_roh = 24d6
    real*8 :: nju12_roh = 0.49
    real*8 :: rho_roh = 52
    ! real*8 :: s_tens_roh = 1.6d6
    ! real*8 :: s_compr_roh = 0.8d6
    ! real*8 :: sLT_roh = 0.8d6
    ...
    real*8, dimension(21) :: E1_layer_spar, E2_layer_spar, G12_layer_spar
    real*8, dimension(21) :: theta_layer_spar, nju12_layer_spar, rho_spar
    real*8, dimension(21) :: t_layer_spar
    ...
    real*8 :: t_cap, t_spar, z0, Ex_spar, core_thickness
    ...
    E1_layer_spar(n_layer_spar+1) = E_roh
    E2_layer_spar = E2_woven
    E2_layer_spar(n_layer_spar+1) = E_roh
    G12_layer_spar = G12_woven
    G12_layer_spar(n_layer_spar+1) = G12_roh
    nju12_layer_spar = nju12_woven
    nju12_layer_spar(n_layer_spar+1) = nju12_roh
    rho_spar = rho_282
    rho_spar(n_layer_spar+1) = rho_roh
    t_layer_spar = t_woven
    t_layer_spar(n_layer_spar+1) = core_thickness
    ! Theta_layer_spar
    do i = 1, n_layer_spar
        theta_layer_spar(i) = (-1)**i*pi/4.0d0
    end do
    theta_layer_spar(n_layer_spar + 1) = 0.0d0
    do i = n_layer_spar + 2, 2*n_layer_spar + 1
        theta_layer_spar(i) = (-1)**i*pi/4.0d0
    end do
    ...
    EI = Ex_spar * I_spar + 2.0d0*EI_cap  ! Do cung cua dam - Bending rigidity
    ...
    open(10, file = "stress_beamwithcore.txt", status = "replace", action = "write")
    do i = 1, 101
        write(10, '(F10.2, 1X, F12.2)') x(i), sigma_cap(i)
    end do
    close(10)
    !call system ("python write_stress_result.py")
end program beam_with_core
